...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        C, H, W = input_dim
        F, HF, WF = num_filters, filter_size, filter_size
        H_out = 1 + (H - 2) // 2  # dim after 2x2 max-pool
        W_out = 1 + (W - 2) // 2  # dim after 2x2 max-pool
        self.params["W1"] = weight_scale * np.random.randn(F,C,HF,WF)
        self.params["b1"] = np.zeros(F)
        self.params["W2"] = weight_scale * np.random.randn(F*H_out*W_out,hidden_dim)
